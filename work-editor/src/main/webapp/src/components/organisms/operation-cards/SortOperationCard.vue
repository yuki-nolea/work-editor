      ...
      <div class="operation-view-item-desc">
        <div class="operation-desc">
          <table class="operation-table">
            ...
            <tr>
              <td v-if="member != null"><p><strong>Sort by:</strong></p></td>
              <td v-if="member != null">{{member}}</td>
            </tr>
            <tr>
...
export default defineComponent({
  ...
  setup: (props) =>
  {
    ...
    const symbol = computed(() => props.operation.getArgs().get("symbol")); 
    const order = computed(() => props.operation.getArgs().get("order"));
    const member = computed(() => props.operation.getArgs().get("member")?.replace(/\[.*\]/g, ""));
    const dimension = computed(() => props.operation.getArgs().get("dimension"));

    const variable = computed(() => vPool.get(symbol.value));

    const sortOrder = computed(() => order.value === SortOrder.Asc ? "Ascending order" : "Descending order");

    return { variable, sortOrder, member, dimension, deleteOperation };
  },
});
</script>
