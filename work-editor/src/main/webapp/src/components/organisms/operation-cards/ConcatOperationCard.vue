        ...
        <div class="operation-desc">
          <table class="operation-table">
            <tr>
              ...
              <td v-for="variable in inputVariables"
                :key="variable"
              >
              {{variable.name}},</td>
            </tr>
...
export default defineComponent({
  ...
  setup: (props) =>
  {
    ...
    const inputSymbols = computed(() => props.operation.getArgs().get("inputVariableSymbols"));
    const outputSymbol = computed(() => props.operation.getArgs().get("outputSymbol"));

    //console.log("inputSymbols:", inputSymbols.value);

    const inputVariables = computed(() =>
    {
      const list = [] as AbstractVariable[];

      for(const item of inputSymbols.value)
      {
        list.push(vPool.get(item)!);
      }
      return list;
    });
    ...
    return { inputVariables, outputVariable, deleteOperation };
  },
});
</script>
