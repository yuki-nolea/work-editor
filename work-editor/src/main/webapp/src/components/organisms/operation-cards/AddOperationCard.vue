        ...
        <div class="operation-desc">
          <table class="operation-table">
            <tr>
              ...
              <td>{{inputVariable1.name}}, {{inputVariable2.name}}</td>
            </tr>
            <tr>
...
export default defineComponent({
  ...
  setup: (props) =>
  {
    ...
    const inputSymbol1 = computed(() => props.operation.getArgs().get("inputSymbol1")); 
    const inputSymbol2 = computed(() => props.operation.getArgs().get("inputSymbol2")); 
    const outputSymbol = computed(() => props.operation.getArgs().get("outputSymbol"));

    const inputVariable1 = computed(() => vPool.get(inputSymbol1.value));
    const inputVariable2 = computed(() => vPool.get(inputSymbol2.value));
    const outputVariable = computed(() => vPool.get(outputSymbol.value));

    return { inputVariable1, inputVariable2, outputVariable, deleteOperation };
  },
});
</script>
